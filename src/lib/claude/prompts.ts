...
4. textStyles は画像中の主要な文字ごとに、推定できる書体の種類と特徴を書く。文字が読めない場合は空配列にする
5. principles は専門用語をタグ名(name)にし、その意味を必ず日常語で description に書く。improvements は「〜するとより伝わりやすくなるかもしれません」のように柔らかく述べる
6. 各項目は簡潔にする（冗長な説明を避ける）。principles は最大3つ、improvements と applications は各2つまで。出力JSONは必ず最後まで完結させる
7. 必ず JSON オブジェクトのみを返す。説明文、前置き、Markdown、コードフェンス表記は書かない
...
4. For textStyles, describe the estimated font type and characteristics for each major visible text. If no text is readable, return an empty array.
5. For principles, put the technical term as the tag (name) and always explain its meaning in plain language in description. For improvements, phrase gently (e.g., "it might read more clearly if...").
6. Keep every field concise (avoid verbosity): at most 3 principles, and at most 2 improvements and 2 applications. Always complete the JSON output to the end.
7. Return only a JSON object. No prose, no preamble, no Markdown, no code fences.
